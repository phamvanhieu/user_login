<?php
    ...
    if(isset($_POST['change-pass'])){
        ...
        if (isset($_POST['password']) && isset($_POST['re_password'])) {
            ...
            if ($pass == $re_pass) {
                $result = $userModel->changPass($pass,$_SESSION['user_id']);
                if ($result == true) {
                    array_push($success, "Sửa Thành Công!!!");
...
<div class="container vh-100 d-flex justify-content-center align-items-center">
    <div class="row w-100">
        <div class="col-md-12">
            <form method="POST" class="border rounded bg-light shadow" enctype="multipart/form-data">
                <div class="title-login bg-warning text-center text-white pt-2 pb-2 rounded-top "><h1>Xin Chào <?php echo strip_tags($user_type)." - ".strip_tags($user['fullname']) ?></h1></div>
                <?php 
                ...
                <div class="row">
                    <div class="col-md-12 text-center mb-3">
                        <input type="submit" class="btn btn-info mt-3" name="edit_user" value="Cập Nhật Thông Tin">
                        <input type="submit" name="user-logout" class="btn btn-danger mt-3" value="Logout Tài Khoản">
                        <input type="submit" name="change-pass" class="btn btn-warning mt-3" value="Đổi mật khẩu">
                        <?php 
                        if($user_type == "admin"){
                        ?>
                        <a href="register.php" class="btn btn-success mt-3">Thêm User</a>
                        <a href="listUser.php" class="btn btn-info mt-3">Danh Sách User</a>
                        <?php
                        }
                        ?>
                    </div>
